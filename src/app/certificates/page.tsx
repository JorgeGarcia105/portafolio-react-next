	...
	return (
		...
		>
			...
			>
				{certificates.map((cert, index) => (
					<motion.article
						...
						className="relative perspective group outline-none focus-visible:ring-4 focus-visible:ring-blue-400"
						tabIndex={0}
						aria-label={`Ver detalles del certificado ${cert.title}`}
						role="listitem"
					>
						{/* Flip Card */}
						<motion.div
							className="relative w-full aspect-[4/3] rounded-2xl overflow-hidden shadow-xl bg-white dark:bg-zinc-800 transform-style preserve-3d transition-transform duration-700 group-hover:rotate-y-180 group-focus:rotate-y-180"
							tabIndex={-1}
						>
							{/* Front Face */}
			...
			<style jsx>{`
				...
				.group-focus\\:rotate-y-180:focus {
					transform: rotateY(180deg);
				}
			`}</style>
		</section>
